			...
			ScrollView {
				VStack {
					...
					Section(header: Text("General Scoring"), footer: Text(" ")) {
						VStack(alignment: .leading) {
							BulletedText("Due to COVID-19 and the increase in mail-in and absentee balloting this year, the game will officially end at 11:59 PM on Friday, November 6th. If you believe that a race will be officially called in a state, pick the candidate you feel will be victorious. If you believe that a state's election will still be undecided by that time, choose \"Too Close to Call.\"")
					...
					Section(header: Text("Presidential, Senatorial, and Gubernatorial Races").multilineTextAlignment(.center), footer: Text(" ")) {
						VStack(alignment: .leading) {
							BulletedText("Your score is based on the total number of correct predictions. You raw score will be determined against all other players, with points earned being in the range of 0 - total number of players. Equal points will be awarded to all players who predicted correctly.")
					...
					Section(header: Text("House Races"), footer: Text(" ")) {
						VStack(alignment: .leading) {
							BulletedText("For each state, use the sliding bar to predict the outcome of the House delegation for that state. If you feel that any of the House races will be undecided by the end of the game, leave those seats unallocated as \"Too Close to Call\". Points are awarded proportional to how many seats you correctly predicted relative to all players.")
							BulletedText("For example, if you correctly predicted 5 out of 7 seats in a particular race and the average of all of the players was 4 correct seats, you would earn 5/4, or 1.25, points.")
						}
					}
					Section(header: Text("League Scoring"), footer: Text(" ")) {
						VStack(alignment: .leading) {
							BulletedText("Your score in a particular league is generated in the same manner as your overall score, except points are distributed according to how other players in your league did.")
					...
					Section(header: Text("Senatorial Race Errata"), footer: Text(" ")) {
						VStack(alignment: .leading) {
							...
							BulletedText("Louisiana has a jungle primary; the candidate with the most votes, even if that candidate does not earn 50% of the vote.")
						}
					}
				}
			}
